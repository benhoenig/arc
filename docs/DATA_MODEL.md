...
**How `actual_amount_thb` is populated:** in M4 (as shipped 2026-04-18) it is directly user-editable. **In M4.5 (see §5.3) it becomes trigger-maintained** from `SUM(flip_transactions.amount_thb) WHERE budget_line_id = X AND deleted_at IS NULL`. The inline UI edit is removed at the start of M4.5 in favor of transaction CRUD. Existing typed values (test data from M4 smoke-testing) are **zeroed out** on M4.5 rollout — operators re-enter real spend as transactions with receipts. See `memory/project_m4_5_transactions_and_m10_investor_link.md` for the design rationale.

### 5.3 `flip_transactions`

Ledger of every money event on a flip — inflows (investor deposits, loan disbursements, eventual sale proceeds) and outflows (contractor payments, materials, fees, marketing, refunds). The single source of truth for flip cash balance and budget line actuals once M4.5 ships.
...
CREATE TABLE flip_transactions (
  ...
  -- Only set on outflows that map to a planned budget line. Required
  -- when kind IN ('spend','refund'); must be NULL for every other kind.
  budget_line_id        uuid REFERENCES budget_lines(id) ON DELETE SET NULL,

  date                  date NOT NULL,

  -- Signed: positive = inflow, negative = outflow. A refund against
  -- an outflow is a positive entry tied to the same budget_line_id.
  amount_thb            numeric(14, 2) NOT NULL,

  description           text NOT NULL,

  -- Free-text source for inflows until the investor_id FK lands in M10.
  -- On outflows this can be used for payee names etc., or left NULL.
  source_note           text,

  kind                  text NOT NULL,
    -- 'investor_deposit' | 'loan_disbursement' | 'spend' | 'refund'
    -- | 'sale_proceeds' | 'distribution'
    -- sale_proceeds + distribution are reserved for later milestones
    -- (M9/M10) but live in the enum today so the schema is stable.
...
CREATE INDEX idx_flip_tx_budget_line
  ON flip_transactions(budget_line_id) WHERE budget_line_id IS NOT NULL AND deleted_at IS NULL;
...
**M10 follow-up (ship blocker):** add `investor_id uuid NULL REFERENCES investors(id) ON DELETE SET NULL` + backfill UI that lets admin link each historical `investor_deposit` row (identified by `source_note`) to a real `investors` record. See `memory/project_m4_5_transactions_and_m10_investor_link.md`.

**RLS:** standard org-member SELECT / INSERT / UPDATE / DELETE (same pattern as `budget_lines`).

**Trigger** (see Section 14): after INSERT / UPDATE / DELETE on `flip_transactions`, `recompute_budget_line_actual()` recalculates `budget_lines.actual_amount_thb` for both OLD and NEW `budget_line_id`. UPDATE handles the case where a transaction is re-categorised — old line's actual decreases, new line's actual increases in a single trigger run.

**Receipt storage:** Supabase Storage bucket `budget-receipts`, **private** (not public — receipts can contain sensitive vendor / banking info). Path pattern `{orgId}/{flipId}/{uuid}.{ext}`. Signed-URL flow for reads, same structure as `property-thumbnails` but reads go through a server action that resolves a 5-minute signed URL per view.

**Why signed = true here, public = true on property-thumbnails:** property thumbnails are the flip photos that appear on listings and in internal UI; leaking them would be embarrassing, not dangerous. Receipts show bank transfer confirmations, contractor bank account numbers, invoice totals tied to person names — standard tax / audit-sensitive data. The added signed-URL round-trip per view is the right tradeoff.
...
### 13.4 `flip_cash_summary` (M4.5)

Per-flip cash balance + breakdown by kind. Powers the cash-balance indicator on the flip detail header and the P&L view (M4.6).
...
### 13.5 `investor_position_summary`
...
After INSERT / UPDATE / DELETE on `flip_transactions`, recompute `budget_lines.actual_amount_thb` for every affected line. On UPDATE, both OLD and NEW `budget_line_id` are affected (handles the "re-categorise a transaction to a different line" case atomically).
...
DECLARE
  v_line_id uuid;
BEGIN
  -- Collect every budget_line_id that could need recomputing.
  IF TG_OP = 'INSERT' THEN
    v_line_id := NEW.budget_line_id;
  ELSIF TG_OP = 'DELETE' THEN
    v_line_id := OLD.budget_line_id;
  ELSE -- UPDATE
    -- Handle NEW first; OLD handled by explicit second call.
    v_line_id := NEW.budget_line_id;
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND OLD.budget_line_id IS DISTINCT FROM NEW.budget_line_id
...
Why recompute rather than incremental delta: simpler to reason about, always-correct under soft-delete (a resurrected transaction is just another non-deleted row), no per-case bug surface. Flip budgets have a bounded number of transactions per line (order of 10s, not thousands), so the SUM is cheap.

### 14.6 Listing lead counter
